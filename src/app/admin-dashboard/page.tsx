...
const AdminDashboard = () => {
  ...
  const handleUpdateAI = async () => {
    setUpdatingAI(true)
    setUpdateStatus('Starting AI update process...')
    ...
    try {
      ...
      if (result.success) {
        setUpdateStatus(`✅ AI updated successfully! ${result.summary ? `Processed ${result.summary.filesAttempted} files (${result.summary.successRate} success rate)` : ''}`)
      } else {
        setUpdateStatus(`❌ AI update failed: ${result.message}`)
      }
    } catch (error) {
